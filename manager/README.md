...
- **cTrader Account ID (Pepperstone only)**: a field to view/set `CTRADER_ACCOUNT_ID` in `execution-service-pepperstone/.env` directly from the browser, instead of hand-editing the file. Blank clears it (auto-pick when exactly one account matches `CTRADER_ENVIRONMENT`); otherwise it must be a numeric `ctidTraderAccountId` (get the value from `get_access_token.py`'s output). Saving while the service is running doesn't restart it automatically — restart the service for a changed account ID to take effect.
